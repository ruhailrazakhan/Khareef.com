...
$IS_ADMIN = require_super_admin(); // from auth.php (cookie tg_admin)
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .navLinks a.active{ background:#eef3ff;color:#003580; }
...
<header class="siteHeader">
  <div class="navContainer">
    ...
    <nav class="navLinks">
      ...
      <?php if ($IS_ADMIN): ?>
        <a href="/admin/admin_add_marker.php" class="<?= $current==='admin_add_marker.php'?'active':'' ?>">Add Marker</a>
...
<script>
  ...
  async function loadMarkers(){
    ...
    try {
      ...
      console.log("get_markers.php response:", text);
  ...
  /* ===== Google Places Photos (no storing) ===== */
  const GooglePhotoCache = new Map(); // place_id -> [urls]

  function fetchGooglePhotoUrls(placeId, maxPhotos = 6) {
    return new Promise((resolve) => {
      if (!placeId || !google?.maps?.places || !MAP) return resolve([]);
      if (GooglePhotoCache.has(placeId)) return resolve(GooglePhotoCache.get(placeId));
      ...
      svc.getDetails({ placeId, fields: ['photos'] }, (place, status) => {
        if (status !== google.maps.places.PlacesServiceStatus.OK || !place?.photos?.length) {
          GooglePhotoCache.set(placeId, []);
          return resolve([]);
        }
        const urls = place.photos.slice(0, maxPhotos).map(p => p.getUrl({ maxWidth: 900 }));
        GooglePhotoCache.set(placeId, urls);
        resolve(urls);
      });
    });
  }

  function popupHtml(loc, mergedImages = [], showGoogleAttribution = false){
    const dest = encodeURIComponent(`${loc.lat},${loc.lng}`);
    const t = typeLabel(loc.type);

    const gallery = mergedImages.length ? `
      <div style="display:flex;gap:8px;overflow:auto;padding-bottom:6px;margin-bottom:10px">
        ${mergedImages.map(u => `
          <img src="${esc(u)}" style="width:120px;height:84px;object-fit:cover;border-radius:12px;border:1px solid #e6ebf0" />
        `).join('')}
      </div>
    ` : '';

    const attribution = showGoogleAttribution ? `
      <div style="font-size:12px;color:#5b6573;margin:-6px 0 10px 0">
        Some photos provided by Google
      </div>
    ` : '';
          ...
          <div style="padding:12px;border-top:1px solid #e6ebf0;background:#fbfcfe">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
              <a target="_blank" href="https://www.google.com/maps/dir/?api=1&destination=${dest}&travelmode=driving"
                style="text-decoration:none;padding:10px 12px;border-radius:12px;background:#003580;color:#fff;font-size:12px;font-weight:900;display:inline-flex;gap:8px;align-items:center">🚗 Driving</a>
              <a target="_blank" href="https://www.google.com/maps/dir/?api=1&destination=${dest}&travelmode=walking"
                style="text-decoration:none;padding:10px 12px;border-radius:12px;background:#fff;color:#0b1320;border:1px solid #e6ebf0;font-size:12px;font-weight:900;display:inline-flex;gap:8px;align-items:center">🚶 Walking</a>
            </div>

            <div style="margin-top:10px">
              <a target="_blank" href="https://www.google.com/maps/search/?api=1&query=${dest}"
                style="color:#0057d9;text-decoration:none;font-size:12px;font-weight:800">Open in Google Maps →</a>
  ...
  function renderList(){
    ...
    listEl.innerHTML = filtered.length ? '' : `<div class="empty">No results</div>`;
    ...
    filtered.forEach(l => {
      ...
      const customThumb = (l.images && l.images.length) ? l.images[0] : '';
      const t = typeLabel(l.type);

      div.innerHTML = `
        <div class="thumbWrap">
          <img class="thumb" data-has-custom="${customThumb ? '1':'0'}" src="${esc(customThumb)}" onerror="this.style.display='none'">
          ${IS_ADMIN ? `<a class="editBtn" onclick="event.stopPropagation()" href="/admin/admin_edit_marker.php?id=${Number(l.id)}">Edit</a>` : ``}
        </div>
        ...
        <div class="cardMain">
          ...
          <div class="meta"><span class="tag">${esc(t)}</span></div>
          <p class="desc">${esc(l.short_text || '')}</p>
        </div>
      `;

      // Lazy Google thumbnail if no custom image
      if (!customThumb && Number(l.use_google_photos || 0) === 1 && l.place_id) {
        fetchGooglePhotoUrls(l.place_id, 1).then(urls => {
          if (!urls.length) return;
          const imgEl = div.querySelector('img.thumb');
          if (imgEl && imgEl.getAttribute('data-has-custom') !== '1') imgEl.src = urls[0];
        });
      }
  ...
  window.initMap = async function(){
    ...
    LOCS.forEach(loc => {
      ...
      // Fast popup with custom images, then upgrade with Google photos
      m.addListener('click', async () => {
        setActiveCard(loc.id);
        scrollToCard(loc.id);

        const custom = (loc.images && loc.images.length) ? loc.images : [];
        INFO.setContent(popupHtml(loc, custom, false));
        INFO.open({anchor:m, map: MAP});

        const useGoogle = Number(loc.use_google_photos || 0) === 1 && !!loc.place_id;
        if (!useGoogle) return;

        const googleUrls = await fetchGooglePhotoUrls(loc.place_id, 6);
        if (!googleUrls.length) return;

        const merged = [...custom, ...googleUrls];
        INFO.setContent(popupHtml(loc, merged, true));
        INFO.open({anchor:m, map: MAP});
      });
...
<!-- IMPORTANT: Places library enabled (for Google photos) -->
<script async
  src="https://maps.googleapis.com/maps/api/js?key=<?= htmlspecialchars(GOOGLE_API_KEY, ENT_QUOTES, 'UTF-8') ?>&libraries=places&callback=initMap"></script>
